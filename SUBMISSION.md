...
* **Verify security rejects unauthorized requests (without header):**
  ...
  ```
  *Expected response: `401 Unauthorized` with an error message.*

* **GET All Employees (with key):**
  ```bash
  curl -H "X-API-KEY: employees-r-us-secure-webhook-key" http://localhost:8080/api/v1/employee
  ```
  *Expected response: `200 OK` returning the list of mock employees.*
...
* **POST Create a New Employee (with key):**
  ...
  ```
  *Expected response: `201 Created` returning the newly created employee with their random UUID.*
